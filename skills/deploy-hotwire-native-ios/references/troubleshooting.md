...
| New certificate appears after each CI run | Disposable automatic provisioning creates account-side identities | Stop the lane; inventory fingerprints and dependent profiles; obtain owner approval before revoking confirmed orphans; import reusable signing material |
...
## Diagnose Xcode-versus-TestFlight push in order

When Sandbox receipt works but TestFlight receipt does not, keep the two device registrations separate and move
through these boundaries:

1. **Registration:** after signing in through TestFlight, verify that Rails recorded a Production/TestFlight device.
   No row means the token-registration lifecycle failed before APNs delivery.
2. **Provider preflight:** check secret presence and parseability without printing values. Confirm Production key
   authorization, team, topic/bundle ID, endpoint selection, and background-job execution.
3. **Correlated delivery:** send one owner-approved self-test and follow its correlation ID through the queued job
   and provider response. `BadDeviceToken` suggests endpoint/environment or invalid-token drift;
   `DeviceTokenNotForTopic` suggests topic/team/key-scope drift.
4. **Physical receipt:** provider acceptance is still not phone receipt. Inspect notification authorization/focus
   state and observe foreground, background, terminated, and notification-tap behavior as required.
5. **Artifact:** inspect the exact trusted-lane IPA/archive for Production APNs, team, bundle ID, and domains. If the
   artifact was not retained, retrieve it from the trusted build's protected artifact store or rebuild the approved
   source SHA with the same pinned toolchain and release inputs. Do not substitute a local Development app.
